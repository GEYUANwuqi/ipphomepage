...
export function validateContent(root: string) {
  const load = (file: string) => {
    try {
      ...
      throw new Error(`${file}: 无法读取 JSON：${error instanceof Error ? error.message : error}`);
    }
  };
  const parse = <T>(file: string, schema: { parse: (input: unknown) => T }): T => {
    try {
      return schema.parse(load(file));
    } catch (error) {
  ...
  for (const item of [...people, ...projects, ...events]) {
    ...
    const path = realpathSync(candidate),
      fromRoot = relative(publicRoot, path);
...
export function contentValidationPlugin(): Plugin {
  ...
  return {
    ...
    load(id) {
      const key = (['people', 'projects', 'events'] as const).find(
        key => id === resolve(root, `src/content/${key}.json`)
      );
    ...
    handleHotUpdate(context) {
      if (context.file.startsWith(resolve(root, 'src/content') + sep) && context.file.endsWith('.json'))
        validateContent(root);
    }
  };
}
