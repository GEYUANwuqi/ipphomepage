...
export function makeMockPeople(): Person[] {
  ...
  const titles = ['前社长（演示）', '项目维护者', '技术分享者', '社区伙伴', '热心支持者'];
  const descriptions = [
  ...
  return names.map((name, i) => {
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 96 96"><metadata>IPP_DEV_PEOPLE_ONLY</metadata><rect width="96" height="96" rx="${i === 0 ? 6 : 28}" fill="${color}"/><path d="M23 62V25l17 11h16l17-11v37Q48 82 23 62" fill="#fff7ed"/><path d="M36 47v12m-6-6h12m18-6v12m-6-6h12" stroke="#514966" stroke-width="3" stroke-linecap="round"/><path d="m43 65 5 3 5-3" fill="none" stroke="#514966" stroke-width="2"/></svg>`;
    const count = [4, 2, 0, 1, 3, 5][i % 6];
    return {
      id: `dev-person-${String(i + 1).padStart(2, '0')}`,
      name: `示例 · ${name}`,
      avatar: `data:image/svg+xml,${encodeURIComponent(svg)}`,
      description: i === 11 ? descriptions.join(' ') : descriptions[i % descriptions.length],
      title: i === 0 ? '社长（演示）' : titles[i % titles.length],
      year: 2022 + Math.floor(random() * 5),
      group: i < 8 ? 'core' : 'supporter',
      featured: i < 4,
      lead: i === 0,
      links: Array.from({ length: count }, (_, j) => ({
        label: ['GitHub', '个人博客', '作品集', '知识笔记', '更多创作'][j],
        url: `https://example.com/demo/member-${i + 1}/link-${j + 1}`
      }))
    };
  });
}
