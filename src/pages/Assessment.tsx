...
export function Assessment() {
  ...
  if (!id) return <div className="page assessment-intro">
    ...
    <div className="intro-layout"><section className="intro-copy"><span className="page-symbol peach"><Fingerprint size={30} /></span><div className="eyebrow muted">A LITTLE REFLECTION, A BETTER COMMUNITY</div><h1>好的社区，<br />从彼此理解开始。</h1><p className="lead">这不是给一个人贴标签的考试，<br />而是一次关于尊重、协作与安全的共识确认。</p><div className="intro-features"><div><Shuffle /><section><h3>每次开始，都有新思考</h3><p>服务端从题库随机抽取，不重复出题。</p></section></div><div><ShieldCheck /><section><h3>公平评分，清晰规则</h3><p>客观题自动评分；简答与量表不计分。</p></section></div><div><Award /><section><h3>记录这一次成长</h3><p>正式审核通过后，可下载带验真二维码的证书。</p></section></div></div></section>
      <section className="surface-card start-card"><div className="card-top"><span className="chip"><span className="status-dot" />{config?.enabled ? '正式审核已开放' : '演示体验'}</span><span className="subtle">无需注册</span></div><h2>准备好，开始 +1</h2><p>留一个昵称，带上你的认真，就可以出发。</p>
      ...
      </section></div><div className="intro-bottom">认真对待每一次交流，就是社区变好的开始。<span>✳</span></div>
  </div>;
  ...
  return <div className="page quiz-page"><div className="breadcrumb"><Link to="/assessment">素质问卷</Link><ChevronRight size={16} />{attempt.demo ? '演示答题' : '正式答题'}</div><div className="quiz-layout"><aside className="quiz-sidebar"><span className="page-symbol lavender"><Fingerprint size={27} /></span><h2>一点思考，<br />多一份共识。</h2><p>{attempt.demo ? '演示模式 · 不签发正式证书' : '正式审核 · 服务端评分'}</p><div className="sidebar-divider" /><div className="progress-label"><span>计分题进度</span><strong>{completed} / {required}</strong></div><progress value={completed} max={required} aria-label="计分题完成进度" /><div className="question-dots">{attempt.questions.map((item, i) => <button key={item.id} className={`${i === index ? 'current' : ''} ${answers[item.id] !== undefined ? 'answered' : ''}`} onClick={() => move(i)} aria-label={`第 ${i + 1} 题${answers[item.id] !== undefined ? '，已作答' : ''}`} aria-current={i === index ? 'step' : undefined}>{i + 1}</button>)}</div><p className="save-state" aria-live="polite">{saveState || '答案将自动保存'}</p><p className="small subtle">有效至 {date(attempt.expiresAt)}<br />多选题须全部选对才得分，无倒扣。<br />提交后不可修改答案。</p></aside>
