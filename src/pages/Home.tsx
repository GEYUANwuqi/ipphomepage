...
export function Home() {
  ...
  return <div className="home page">
    ...
    {!!featuredPeople.length && <section className="home-people" aria-labelledby="home-people-title"><div className="section-heading"><div><h2 id="home-people-title">成员与支持者<PawMark /></h2></div><Link className="text-link" to="/people">查看完整书架 <ArrowRight size={18} /></Link></div>{isDemo && <DemoPeopleNotice />}<div className="people-grid featured-people-grid">{featuredPeople.map(p => <PersonBook key={p.id} person={p} />)}</div></section>}
