...
export function readPreferences(): ThemePreferences {
  ...
  try {
    mode = localStorage.getItem('ipp-theme');
    seed = localStorage.getItem('ipp-seed');
  } catch {
    /* Private browsing may deny storage. */
  }
  return {
    dark: mode === 'dark' || (mode !== 'light' && matchMedia('(prefers-color-scheme: dark)').matches),
    seed: normalizeSeed(seed) ?? DEFAULT_SEED
  };
}
...
function paletteCss(seed: string) {
  if (!styles.has(seed)) {
    const declarations = (dark: boolean) =>
      Object.entries(buildPalette(seed, dark))
        .map(([key, value]) => `${key}:${value}`)
        .join(';');
    // Both modes are present, so CSS media/debug tools and browser snapshots see complete schemes.
    styles.set(
      seed,
      `:root,:root[data-theme="light"]{${declarations(false)}}:root[data-theme="dark"]{${declarations(true)}}`
    );
...
export function applyPreferences(theme: ThemePreferences, persist = true) {
  ...
  if (!style) {
    ...
    style.id = 'ipp-dynamic-palette';
    document.head.append(style);
  }
  if (style.dataset.seed !== theme.seed) {
    style.textContent = paletteCss(theme.seed);
    style.dataset.seed = theme.seed;
  }
  ...
  if (persist) {
    try {
      localStorage.setItem('ipp-theme', theme.dark ? 'dark' : 'light');
      localStorage.setItem('ipp-seed', theme.seed);
    } catch {
      /* The in-memory theme still works. */
    }
  }
}
