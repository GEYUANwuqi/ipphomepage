...
export function usePageMotion(main: RefObject<HTMLElement | null>, pathname: string) {
  useLayoutEffect(() => {
    ...
    const entrance = main.current.animate([
      { opacity: 0, transform: `translateY(${compact ? 16 : 24}px) scale(${compact ? '.992' : '.985'})`, filter: compact ? 'none' : 'blur(5px)', clipPath: `inset(0 0 ${compact ? 10 : 18}px 0 round ${compact ? 16 : 24}px)` },
      { opacity: 1, transform: 'translateY(0) scale(1)', filter: compact ? 'none' : 'blur(0)', clipPath: 'inset(0 0 0 0 round 0)' },
    ...
    return () => { entrance.cancel(); media.removeEventListener('change', cancel); };
  }, [pathname, main]);
}
...
export function useArtParallax() {
  ...
  useEffect(() => {
    ...
    return () => { reset(); media.current?.removeEventListener('change', reset); };
  }, []);
