...
- `src/components/Mascot.tsx`：参考灰紫发、猫耳、蓝眼、金铃与尾巴绘制的 **简化试作**。不是参考照片中的完整重绘，后续获得透明原稿后可替换。
...
生产测试扫描编译产物确认没有 mock 模块/内联头像，并在临时目录构建虚构成员/赛事夹具来验证只编辑 JSON 和图片即可展示，结束后销毁临时数据，不改动正式名录。发布前还应人工核对实际头像裁剪、链接目的地、长名字、窄屏书签和站娘形象是否合适。
