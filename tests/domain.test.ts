...
test('random sampling is without replacement, scored, and varies across draws', () => {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    assert.equal(qs.length, 8); assert.equal(new Set(qs.map(q => q.id)).size, 8);
  ...
  assert.throws(() => drawQuestions(seedQuestions.filter(q => q.points === 0), 1));
});
test('all objective types score server answers; optional answers never affect score', () => {
  ...
  a['reflection-1'] = '这是一份未评分的简答'; a['reflection-2'] = 1;
...
test('invalid, foreign, duplicate, incomplete and out-of-range responses are rejected', () => {
  ...
  for (const a of [{}, { [q.id]: [] }, { [q.id]: [0, 0] }, { [q.id]: [900] }, { [q.id]: [0], foreign: [0] }, { [q.id]: '0' }]) {
...
test('schema rejects scored subjective questions and invalid objective keys', () => {
  ...
  const q = seedQuestions[0];
  for (const bad of [ { ...q, correct: [7] }, { ...q, correct: [0, 1] }, { ...q, options: ['a', 'a'] }, { ...q, points: 0 }, { ...q, type: 'text', points: 10 } ]) assert.equal(questionSchema.safeParse(bad).success, false);
});
test('weighted scores use original objective point denominator', () => {
  const questions: Question[] = [{ ...seedQuestions[0], id: 'a', points: 1 }, { ...seedQuestions[0], id: 'b', points: 2 }];
  const r = grade(questions, { a: [1], b: [0] });
  assert.equal(r.score, 66.67); assert.equal(r.earned, 2); assert.equal(r.total, 3);
});
