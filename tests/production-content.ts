...
function assertNoMocks(dist: string) {
  for (const file of readdirSync(dist, { recursive: true }) as string[]) {
    ...
    assert.ok(!text.includes('IPP_DEV_PEOPLE_ONLY') && !text.includes('dev-person-') && !text.includes('示例 · 星屿'), `Development data leaked into ${file}`);
...
const production = createApp({ dbPath: ':memory:', production: true, origin: 'https://iplusplus.club', adminPassword: 'isolated-production-test-password' });
...
try {
  ...
  await page.addInitScript(() => { (window as any).cspErrors = []; document.addEventListener('securitypolicyviolation', e => (window as any).cspErrors.push(e.violatedDirective)); });
  for (const path of ['/', '/people', '/projects', '/events']) {
    assert.equal((await page.goto(server.url + path))?.status(), 200);
    await expect(page.locator('main h1')).toBeVisible();
    const expectedPeople = path === '/' ? published.people.filter(p => p.featured).slice(0, 4).length : path === '/people' ? published.people.length : 0;
  ...
  cpSync('src', join(fixtureRoot, 'src'), { recursive: true }); cpSync('shared', join(fixtureRoot, 'shared'), { recursive: true });
  cpSync('public', join(fixtureRoot, 'public'), { recursive: true }); cpSync('index.html', join(fixtureRoot, 'index.html'));
  ...
  writeFileSync(join(fixtureRoot, 'public/images/people/fixture.svg'), '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" fill="#c9bddf"/></svg>');
  writeFileSync(join(fixtureRoot, 'src/content/people.json'), JSON.stringify({ people: [{ id: 'production-fixture', name: '生产夹具伙伴', avatar: '/images/people/fixture.svg', description: '仅用于自动化测试，不是真实成员。', title: '测试身份', year: 2025, group: 'supporter', featured: true, links: [{ label: '测试链接', url: 'https://example.com' }] },
    { id: 'fixture-long-profile', name: 'N'.repeat(80), avatar: '/images/people/fixture.svg', description: 'D'.repeat(3000), title: 'T'.repeat(80), year: 2026, group: 'core', links: Array.from({ length: 4 }, (_, i) => ({ label: 'Bookmark'.repeat(5), url: `https://example.com/long-${i}` })) },
  ] }));
  writeFileSync(join(fixtureRoot, 'src/content/events.json'), JSON.stringify({ events: [
    { id: 'fixture-past', name: '承办样例（仅测试）', summary: '仅在临时构建中验证赛事显示。', role: 'organizer', status: 'completed', startDate: '2025-05-01', endDate: '2025-05-03', location: '测试场地', links: [{ label: '作品归档', url: 'https://example.com/results' }] },
    { id: 'fixture-next', name: '联合主办样例（仅测试）', summary: '未在真实网站发布的测试记录。', role: 'cohost', status: 'upcoming', startDate: '2030-06-01' },
  ] }));
  ...
  try {
    ...
    for (const width of [320, 1440]) {
      ...
      const bounds = await longBook.evaluate(book => ({ bottom: book.getBoundingClientRect().bottom, bookmarks: book.querySelector('.person-bookmarks')!.getBoundingClientRect().bottom }));
    ...
    await page.goto(fixture.url + '/events'); await expect(page.locator('.event-card')).toHaveCount(2);
    ...
    await page.getByRole('button', { name: '即将开始', exact: true }).click(); await expect(page.locator('.event-card')).toHaveCount(1);
    await expect(page.getByRole('heading', { name: '联合主办样例（仅测试）' })).toBeVisible();
    await page.getByRole('button', { name: '进行中', exact: true }).click(); await expect(page.getByRole('heading', { name: '这个分类还没有赛事。' })).toBeVisible();
    await page.close();
  } finally { await fixture.close(); }
  console.log('Production content passed: no dev mocks; empty states; production CSP/theme; JSON-only member/avatar addition; event listing, dates, roles, links and filters.');
} finally {
  await browser.close(); await server.close(); production.db.close(); rmSync(fixtureRoot, { recursive: true, force: true });
}
