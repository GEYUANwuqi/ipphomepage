...
const localImage = text(300)
  .regex(
    /^\/images\/(people|projects|events)\/.+\.(png|jpe?g|webp|avif|svg)$/i,
    '图片必须位于 /images/people、projects 或 events 下'
  )
  .refine(
    value =>
      !/[\\%?#\u0000-\u001f]/.test(value) &&
...
export const projectSchema = z
  .object({
    ...
    name: text(100),
    summary: text(1000),
    category: z.enum(['engine', 'language', 'framework', 'game']),
    relationship: text(200),
    tags: z.array(text(30)).max(8),
    featured: z.boolean().default(false),
    cover: localImage.optional(),
    coverAlt: text(200).optional(),
    illustration: z.enum(['engine', 'code', 'story', 'game']).optional(),
    links,
    licenseNote: text(600),
    sourceUrl: url
  })
  .strict()
  .refine(p => !!p.coverAlt || !p.cover, '提供封面时必须填写 coverAlt');
const isoDate = z
  .string()
...
function unique<T extends { id: string }>(items: T[], ctx: z.RefinementCtx) {
  ...
  items.forEach((item, i) => {
    if (seen.has(item.id)) ctx.addIssue({ code: 'custom', path: [i, 'id'], message: `重复 id：${item.id}` });
    seen.add(item.id);
  });
}
